...
public class LuceneUtil {
	 ...
	 * @erikchang
	 */
	public List<LuceneVo> searchIndex(String keyword, int start, int size,
			double begin_price, double end_price, Sort sort) throws IOException {
		...
		try {
			...
			for (int i = start; i < end; i++) {
				...
				if (content == null) {
					vo.setVo_content(doc.get(LuceneVo.CONTENT));
				} else {
				...
				// if (content != null)
				// content = vo.getVo_content().replaceAll(
				// suffixHTML + prefixHTML, "");
				// 实体赋值
				vo.setVo_id(CommUtil.null2Long(doc.get(LuceneVo.ID)));
				vo.setVo_title(title);
				vo.setVo_url(doc.get(LuceneVo.URL));
	 ...
	 * @return
	 */
	public LuceneResult search(String params, int pageNo, double begin_price,
			double end_price, ScoreDoc after, Sort sort) {
		...
		try {
			...
			if (intPageNo < 1)
				intPageNo = 1;
			List<LuceneVo> vo_list = this.searchIndex(params, (intPageNo - 1)
	...
	public static void main(String[] args) {
		...
		Date d1 = new Date();
		LuceneResult list = lucence.search("专柜正品黑色时尚冬装男", 0, 0, 500.0, null, null);
		Date d2 = new Date();
