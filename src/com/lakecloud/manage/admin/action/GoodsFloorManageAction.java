...
import com.lakecloud.foundation.domain.query.GoodsFloorQueryObject;
...
import com.lakecloud.manage.admin.tools.GoodsFloorTools;
...
public class GoodsFloorManageAction {
	...
	@SecurityMapping(title = "楼层logo图", value = "/admin/goods_floor_logo.htm*", rtype = "admin", rname = "首页楼层", rcode = "goods_floor", rgroup = "商品")
	@RequestMapping("/admin/goods_floor_logo.htm")
	public ModelAndView goods_floor_logo(HttpServletRequest request,
			HttpServletResponse response, String currentPage, String id) {
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_class.html",
				configService.getSysConfig(),
	...
	public String goods_floor_right_adv_save(HttpServletRequest request,
			HttpServletResponse response, String type, String id,
			String adv_url, String adv_id ,String ids) {
		GoodsFloor obj = this.goodsfloorService.getObjById(CommUtil
				.null2Long(id));
		Map map = new HashMap();
		if (type.equals("user")) {
		...
		if(type.equals("shop")){
			String[] id_list = ids.split(",");
			Map shop_map = new HashMap();
			
			for (int i = 0; i < id_list.length; i++) {
				if (!id_list[i].equals("")) {
					shop_map.put("shop_id" + i, id_list[i]);
				}
			}
			obj.setGf_right_adv(Json.toJson(shop_map, JsonFormat.compact()));//保存数据信息
		}
		this.goodsfloorService.update(obj);
		return "redirect:goods_floor_template.htm?id=" + obj.getId();
	}
	...
	@SecurityMapping(title = "楼层模板分类店铺编辑", value = "/admin/goods_floor_list_shop_load.htm*", rtype = "admin", rname = "首页楼层", rcode = "goods_floor", rgroup = "商品")
	@RequestMapping("/admin/goods_floor_list_shop_load.htm")
	public ModelAndView goods_floor_list_shop_load(HttpServletRequest request,
			HttpServletResponse response, String currentPage, String sc_id,
			String shop_name) {
 
		ModelAndView mv = new JModelAndView(
				"admin/blue/goods_floor_list_shop_load.html",
				configService.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		StoreQueryObject qo = new StoreQueryObject(currentPage, mv, "addTime",
				"desc");
		if (!CommUtil.null2String(sc_id).equals("")) {
			Set<Long> ids = this.genericIds(this.storeClassService
					.getObjById(CommUtil.null2Long(sc_id)));
			Map paras = new HashMap();
			paras.put("ids", ids);
			qo.addQuery("obj.sc.id in (:ids)", paras);
		}
		if (!CommUtil.null2String(shop_name).equals("")) {
		...
		CommUtil.saveIPageList2ModelAndView(CommUtil.getURL(request)
				+ "/admin/goods_floor_list_shop_load.htm", "", "&gc_id="
				+ sc_id + "&shop_name=" + shop_name, pList, mv);
		return mv;
	}
