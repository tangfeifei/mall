...
public class StoreManageAction {
	...
	public ModelAndView store_save(HttpServletRequest request,
		...
		if (!id.equals("") && store.getStore_status() == 3) {
			this.send_site_msg(request, "msg_toseller_store_closed_notify",
					store);
		}
	...
	private void send_site_msg(HttpServletRequest request, String mark,
		...
		if (template.isOpen()) {
			String path = request.getRealPath("/") + File.separator + "vm"
					+ File.separator;
			if (!CommUtil.fileExist(path + "msg.vm")) {
				File file = new File(path + "msg.vm");
			}
