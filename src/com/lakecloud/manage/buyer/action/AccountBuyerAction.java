...
public class AccountBuyerAction {
	...
	public ModelAndView account_nav(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/account_nav.html", configService
						.getSysConfig(),
	...
	public ModelAndView account(HttpServletRequest request,
			HttpServletResponse response) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/account.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		mv.addObject("user", this.userService.getObjById(SecurityUserHolder
				.getCurrentUser().getId()));
	...
	public ModelAndView account_password(HttpServletRequest request,
			HttpServletResponse response) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/account_password.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		return mv;
	}
	...
	public ModelAndView account_email(HttpServletRequest request,
			HttpServletResponse response) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/account_email.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		return mv;
	}
	...
	public ModelAndView account_avatar(HttpServletRequest request,
			HttpServletResponse response) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/account_avatar.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		mv.addObject("user", this.userService.getObjById(SecurityUserHolder
				.getCurrentUser().getId()));
	...
	public ModelAndView account_mobile(HttpServletRequest request,
			HttpServletResponse response) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/account_mobile.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		mv.addObject("url", CommUtil.getURL(request));
		return mv;
	...
	public ModelAndView account_mobile_save(HttpServletRequest request,
		...
		if (mvc != null && mvc.getCode().equalsIgnoreCase(mobile_verify_code)) {
			...
			// 绑定成功后发送手机短信提醒
			this.send_sms(request, "sms_tobuyer_mobilebind_notify");
			mv
	...
	public ModelAndView account_friend(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/account_friend.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
	...
	public ModelAndView friend_search(HttpServletRequest request,
			HttpServletResponse response, String userName, String area_id,
			String sex, String years, String currentPage) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/account_friend_search.html",
				configService.getSysConfig(), this.userConfigService
						.getUserConfig(), 0, request, response);
		...
		if (years != null && !years.equals("")) {
			...
			if (years.equals("18")) {
				qo.addQuery("obj.years", new SysMap("years", CommUtil
						.null2Int(years)), "<=");
			}
			if (years.equals("50")) {
				qo.addQuery("obj.years", new SysMap("years", CommUtil
			...
			if (!years.equals("18") && !years.equals("50")) {
				...
				qo.addQuery("obj.years", new SysMap("years", CommUtil
						.null2Int(y[0])), ">=");
				qo.addQuery("obj.years", new SysMap("years2", CommUtil
						.null2Int(y[1])), "<=");
			}
		}
		if (sex != null && !sex.equals("")) {
			mv.addObject("sex", sex);
			qo.addQuery("obj.sex", new SysMap("sex", CommUtil.null2Int(sex)),
					"=");
		}
		if (area_id != null && !area_id.equals("")) {
			Area area = this.areaService
			...
			qo.addQuery("obj.area.id", new SysMap("area_id", CommUtil
					.null2Long(area_id)), "=");
		}
		qo.setPageSize(18);
		qo.addQuery("obj.id", new SysMap("user_id", SecurityUserHolder
				.getCurrentUser().getId()), "!=");
	...
	public void friend_add_save(HttpServletRequest request,
		...
		List<SnsFriend> sfs = this.sndFriendService
				.query(
		...
		if (sfs.size() == 0) {
			...
			friend.setToUser(this.userService.getObjById(CommUtil
					.null2Long(user_id)));
	...
	public ModelAndView account_bind(HttpServletRequest request,
			HttpServletResponse response) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/account_bind.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder
				.getCurrentUser().getId());
	...
	public String account_bind_cancel(HttpServletRequest request,
		...
		if(CommUtil.null2String(account).equals("mobile")){
			user.setMobile(null);
		}
