...
import com.lakecloud.core.domain.virtual.CglibBean;
...
public class TransportTools {
	...
	public String query_transprot(String json, String mark) {
		String ret = "";
		List<Map> list = Json.fromJson(ArrayList.class, json);
			...
			for (Map map : list) {
				...
				for (String city : city_list) {
					...
					if (city.equals(city_name) || city_name.indexOf(city) == 0) {// 城市匹配，如“沈阳市”和“沈阳”是一致的
						...
						if (trans_type == 1) {// 按照体积计算运费用
							if (CommUtil.null2Float(goods_weight) > 0) {
								fee = trans_fee;
								float other_price = 0;
								if (trans_add_weight > 0) {
									other_price = trans_add_fee
						...
						if (trans_type == 2) {// 按照体积计算运费用
							if (CommUtil.null2Float(goods_volume) > 0) {
								fee = trans_fee;
								float other_price = 0;
								if (trans_add_weight > 0) {
									other_price = trans_add_fee
