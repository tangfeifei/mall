...
import com.lakecloud.foundation.domain.query.AdvertPositionQueryObject;
import com.lakecloud.foundation.domain.query.AdvertQueryObject;
...
public class AdvertSellerAction {
	...
	public ModelAndView advert_list(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/advert_list.html", configService
						.getSysConfig(),
	...
	public ModelAndView advert_apply(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/advert_apply.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
				...
				try {
					...
					if (fileName.equals("")) {
						if (map.get("fileName") != "") {
							...
							acc.setSize(CommUtil
									.null2Float(map.get("fileSize")));
	...
	public ModelAndView advert_my(HttpServletRequest request,
			HttpServletResponse response, String currentPage) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/advert_my.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		AdvertQueryObject qo = new AdvertQueryObject(currentPage, mv,
				"addTime", "desc");
		qo.addQuery("obj.ad_user.id", new SysMap("ad_user", SecurityUserHolder
				.getCurrentUser().getId()), "=");
	...
	public ModelAndView advert_delay(HttpServletRequest request,
			HttpServletResponse response, String id) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/advert_delay.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		Advert obj = this.advertService.getObjById(CommUtil.null2Long(id));
		User user = this.userService.getObjById(SecurityUserHolder
