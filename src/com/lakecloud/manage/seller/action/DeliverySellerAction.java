...
import com.lakecloud.foundation.domain.query.DeliveryLogQueryObject;
...
public class DeliverySellerAction {
	...
	public ModelAndView delivery(HttpServletRequest request,
		...
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/delivery.html", configService
						.getSysConfig(),
		...
		qo.addQuery("obj.d_goods.goods_store.id", new SysMap("store_id", user
				.getStore().getId()), "=");
	...
	public ModelAndView delivery_log(HttpServletRequest request,
			HttpServletResponse response, String currentPage, String orderBy,
			String orderType) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/delivery_log.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder
				.getCurrentUser().getId());
	...
	public ModelAndView delivery_buy(HttpServletRequest request,
			HttpServletResponse response) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/delivery_buy.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder
				.getCurrentUser().getId());
	...
	public String delivery_buy_save(HttpServletRequest request,
	...
	public ModelAndView delivery_buy_error(HttpServletRequest request,
			HttpServletResponse response) {
	...
	public ModelAndView delivery_apply(HttpServletRequest request,
			HttpServletResponse response, String id) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/delivery_apply.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder
				.getCurrentUser().getId());
		...
		int minDate = CommUtil.null2Int(map.get("day"));// 发布买就送距今天最短日期
		minDate = minDate > 0 ? minDate : 0;
		map.clear();
		map = CommUtil.cal_time_space(new Date(), store.getDelivery_end_time());
		int maxDate = CommUtil.null2Int(map.get("day")) + 1;// 发布买就送距今天最长日期
		...
		if (!CommUtil.null2String(id).equals("")) {
			DeliveryGoods obj = this.deliveryGoodsService.getObjById(CommUtil
					.null2Long(id));
	...
	public ModelAndView delivery_goods(HttpServletRequest request,
			HttpServletResponse response, String goods_name,
			String currentPage, String node_id) {
		ModelAndView mv = new JModelAndView(
				"user/default/usercenter/delivery_goods.html", configService
						.getSysConfig(),
				this.userConfigService.getUserConfig(), 0, request, response);
		User user = this.userService.getObjById(SecurityUserHolder
				.getCurrentUser().getId());
		...
		qo.addQuery("obj.activity_status", new SysMap("activity_status",0), "=");//无参加商城活动的商品
		qo.addQuery("obj.combin_status", new SysMap("combin_status",0), "=");//无参加买就送的商品
		qo.setPageSize(15);
	...
	public ModelAndView delivery_apply_save(HttpServletRequest request,
		...
		if (!delivery_session1.equals("")
			...
			DeliveryGoods obj = wf.toPo(request, DeliveryGoods.class);
			obj.setAddTime(new Date());
			Goods d_goods = this.goodsService.getObjById(CommUtil
					.null2Long(main_goods_id));
			obj.setD_goods(d_goods);
			Goods d_delivery_goods = this.goodsService.getObjById(CommUtil
					.null2Long(give_goods_id));
