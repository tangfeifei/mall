...
/**
 * Discuz! Ucenter API for lakecloud,UC Client 通用调用该类来实现与UC Server之间的通信。 该类实现与UC
 * Server通信的所有接口函数,www.chinacloud.net,erikchang
 * 
 */
public class UCClient extends PHPFunctions {
	...
	public static String UC_KEY = "123456";// lakecloud应用的KEY
	public static String UC_APPID = "2";// lakecloud应用的ID
	...
	public String uc_authcode(String string, String operation, String key,
		...
		if (operation.equals("DECODE")) {
			...
			if ((CommUtil.null2Int(substr(result.toString(), 0, 10)) == 0 || CommUtil
					.null2Long(substr(result.toString(), 0, 10))
					- time() > 0)
	...
	public String uc_user_login(String username, String password, int isuid,
			int checkques, int questionid, String answer) {
		Map<String, Object> args = new HashMap<String, Object>();
		args.put("username", username);
		args.put("password", password);
	 ...
	 *            email Email
	 * @param int
	 *            ignoreoldpw 是否忽略旧密码, 忽略旧密码, 则不进行旧密码校验.
	 * @param int
	 *            questionid 安全提问
	 * @param string
	 *            answer 安全提问答案
	...
	public String uc_user_edit(String username, String oldpw, String newpw,
			String email, int ignoreoldpw, int questionid, int answer) {
		Map<String, Object> args = new HashMap<String, Object>();
		args.put("username", username);
