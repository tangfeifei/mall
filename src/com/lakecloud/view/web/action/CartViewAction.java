...
public class CartViewAction {
	...
	public ModelAndView goods_cart1(HttpServletRequest request,
		...
		if (this.configService.getSysConfig().isZtc_status()) {
			...
			ztc_goods = this.randomZtcGoods(goods);
	...
	public ModelAndView goods_cart3(HttpServletRequest request,
			...
			String addr_id, String coupon_id, String transport)
			throws Exception {
		...
		if (cart != null) {
			if (CommUtil.null2String(cart_session1).equals(cart_session)) {// 禁止重复提交订单信息
				...
				of.setGoods_amount(store_cart.getTotal_price());// 商品总价格需要根据购物车信息获取，不能通过前台传递，防止用户恶意修改页面数据信息
				List<SysMap> sms = this.transportTools.query_cart_trans(
						store_cart,
						CommUtil.null2String(addr.getArea().getId()));
				if (!CommUtil.null2String(transport).equals("平邮")
						&& !CommUtil.null2String(transport).equals("快递")
						&& !CommUtil.null2String(transport).equals("EMS")) {// 如果用户恶意修改快递信息，曾默认为快递
					transport = "快递";
				}
				for (SysMap sm : sms) {
					if (CommUtil.null2String(sm.getKey()).indexOf(transport) >= 0) {
						of.setShip_price(BigDecimal.valueOf(CommUtil
								.null2Float(sm.getValue())));
					}
				}
				...
				if (!CommUtil.null2String(coupon_id).equals("")) {
					...
					if (ci.getUser()
							.getId()
							.equals(SecurityUserHolder.getCurrentUser().getId())) {// 判断优惠券是否当前用户所拥有的，避免用户修改前端数据
						ci.setStatus(1);
