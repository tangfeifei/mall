...
public class PayViewAction {
	...
	public ModelAndView aplipay_return(HttpServletRequest request,
		...
		if (type.equals("store_deposit")) {
			spd = this.storeDepositLogService.getObjById(CommUtil
					.null2Long(order_no));
		}
		// 获取支付宝GET过来反馈信息
		...
		if (type.equals("cash") || type.equals("gold")
			...
			if (type.equals("store_deposit")) {
				q_params.put("mark", spd.getDp_payment_mark());
			}
		...
		if (verify_result) {// 验证成功
				...
				if (trade_status.equals("WAIT_SELLER_SEND_GOODS")
					...
					if (order.getOrder_status() != 20) {// 异步没有出来订单，则同步处理订单
						...
						if (this.configService.getSysConfig()
								.getConfig_payment_type() == 1) {// 平台支付方式，卖家预存款增加并记录日志
							User user = this.userService.getObjById(order
									.getStore().getUser().getId());
							...
							log.setPd_log_info("订单：" + order.getOrder_id()
									+ "通过平台支付交易完成，预存款增加");
			...
			if (type.equals("cash")) {
				if (trade_status.equals("WAIT_SELLER_SEND_GOODS")
						|| trade_status.equals("TRADE_FINISHED")
						|| trade_status.equals("TRADE_SUCCESS")) {
					if (pd.getPd_pay_status() != 2) {// 异步没有处理该充值业务，则同步处理一下
						pd.setPd_status(1);
			...
			if (type.equals("store_deposit")) {// 店铺保证缴纳
				if (trade_status.equals("WAIT_SELLER_SEND_GOODS")
						|| trade_status.equals("TRADE_FINISHED")
						|| trade_status.equals("TRADE_SUCCESS")) {
					if (spd.getDp_status() != 10) {
						spd.setDp_status(10);
						this.storeDepositLogService.update(spd);
						User store_user = this.userService.getObjById(spd
								.getDp_user_id());
						store_user.setStore_deposit_status(10);
						store_user.setStore_deposit(spd.getDp_amount());
	...
	public void alipay_notify(HttpServletRequest request,
		...
		String subject = new String(request.getParameter("subject").getBytes(
				"ISO-8859-1"), "UTF-8");// request.getParameter("subject");
		// 商品名称、订单名称
		String order_no = order_nos[2];
		String type = CommUtil.null2String(request.getParameter("body")).trim();
		String trade_status = request.getParameter("trade_status"); // 交易状态
					...
					if (this.configService.getSysConfig()
						...
						BigDecimal addMoney = BigDecimal.valueOf(CommUtil
								.subtract(order.getTotalPrice(),
										order.getShip_price()));
						user.setAvailableBalance(BigDecimal.valueOf(CommUtil
								.add(user.getAvailableBalance(), addMoney)));
						this.userService.update(user);
						PredepositLog log = new PredepositLog();
						log.setAddTime(new Date());
						log.setPd_log_amount(addMoney);
	...
	public void tenpay(HttpServletRequest request,
		...
		if (type.equals("cash")) {
			order_price = CommUtil.null2String(pd.getPd_amount());
		}
		if (type.equals("gold")) {
		...
		if (type.equals("store_deposit")) {
			order_price = CommUtil.null2String(deposit.getDp_amount());
		}
		/* 商品价格（包含运费），以分为单位 */
		...
		if (type.equals("cash")) {
			remarkexplain = pd.getPd_remittance_info();
		}
		if (type.equals("gold")) {
		...
		if (type.equals("store_deposit")) {
			attach = type + "," + deposit.getId().toString();
		}
		...
		if (type.equals("store_deposit")) {
			deposit.setDp_no("sdp-" + trade_no + deposit.getId().toString());
			boolean flag = this.storeDepositLogService.update(deposit);
			if (flag) {
	...
	public ModelAndView tenpay_return(HttpServletRequest request,
		...
		if (attachs[0].equals("cash")) {
			pd = this.predepositService.getObjById(CommUtil
					.null2Long(attachs[1]));
			Map q_params = new HashMap();
			q_params.put("install", true);
			q_params.put("mark", pd.getPd_payment());
		...
		if (attachs[0].equals("store_deposit")) {
			deposit = this.storeDepositLogService.getObjById(CommUtil
					.null2Long(attachs[1]));
			resHandler.setKey(deposit.getDp_payment_mark());
		}
				...
				if ("0".equals(trade_state)) {
					...
					if (attachs[0].equals("cash")) {
						...
						mv = new JModelAndView("success.html",
								...
								request, response);
						mv.addObject("op_title", "充值" + pd.getPd_amount()
								+ "成功");
						mv.addObject("url", CommUtil.getURL(request)
								+ "/buyer/predeposit_list.htm");
					}
					...
					if (attachs[0].equals("store_deposit")) {
						if (deposit.getDp_status() != 10) {
							deposit.setDp_status(10);
							this.storeDepositLogService.update(deposit);
							User store_user = this.userService
									.getObjById(deposit.getDp_user_id());
							store_user.setStore_deposit_status(10);
							store_user.setStore_deposit(deposit.getDp_amount());
							this.userService.update(store_user);
						}
	...
	public ModelAndView chinabank_return(HttpServletRequest request,
		...
		if (remark2.equals("cash")) {
			pd = this.predepositService.getObjById(CommUtil.null2Long(remark1));
		}
		...
		if (remark2.equals("store_deposit")) {
			deposit = this.storeDepositLogService.getObjById(CommUtil
					.null2Long(remark1));
		}
		String key = "";
	...
	public ModelAndView paypal_return(HttpServletRequest request,
		...
		if (paymentStatus.equals("Completed")
			...
			if (remark2.equals("goods")) {
				...
				this.orderFormLogService.save(ofl);
				mv.addObject("obj", order);
				if (this.configService.getSysConfig().getConfig_payment_type() == 1) {// 平台支付方式，卖家预存款增加并记录日志
					User user = this.userService.getObjById(order.getStore()
							.getUser().getId());
					// 需要增加预存款的金额=订单总价-配送金额
					BigDecimal addMoney = BigDecimal.valueOf(CommUtil.subtract(
							order.getTotalPrice(), order.getShip_price()));
					user.setAvailableBalance(BigDecimal.valueOf(CommUtil.add(
							user.getAvailableBalance(), addMoney)));
					this.userService.update(user);
					PredepositLog log = new PredepositLog();
					log.setAddTime(new Date());
					log.setPd_log_amount(addMoney);
			...
			if (remark2.endsWith("cash")) {
				...
				User user = this.userService
						.getObjById(pd.getPd_user().getId());
				user.setAvailableBalance(BigDecimal.valueOf(CommUtil.add(
						user.getAvailableBalance(), pd.getPd_amount())));
				this.userService.update(user);
				PredepositLog log = new PredepositLog();
				...
				mv = new JModelAndView("success.html",
						...
						this.userConfigService.getUserConfig(), 1, request,
						response);
				mv.addObject("op_title", "成功充值：" + pd.getPd_amount());
				mv.addObject("url", CommUtil.getURL(request)
						+ "/buyer/predeposit_list.htm");
			}
			if (remark2.equals("gold")) {
			...
			if (remark2.equals("store_depoist")) {
				if (deposit.getDp_status() != 10) {
					deposit.setDp_status(10);
					this.storeDepositLogService.update(deposit);
					User store_user = this.userService.getObjById(deposit
